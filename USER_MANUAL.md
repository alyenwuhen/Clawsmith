...
3. [Understanding clawsmith](#3-understanding-clawsmith)
...
- **OpenClaw** installed (clawsmith will auto-detect it at `~/.openclaw`)
...
When you first run clawsmith, it looks for OpenClaw at:

1. `~/.openclaw` (default)
2. `OPENCLAW_DIR` environment variable (if set)
3. `clawsmith.openclaw.dir` in `~/.clawsmith/config.json` (if set)
...
```
✓ clawsmith daemon started (pid 12345)
✓ Watching: ~/.openclaw
...
## 3. Understanding clawsmith

### What clawsmith reads

clawsmith reads these OpenClaw files (it never modifies them, except when you explicitly run `memory add/edit/delete`):

| File | What clawsmith does with it |
|------|-----------------------------|
...
### Where clawsmith stores its data

clawsmith keeps its own database at `~/.clawsmith/probe.db`. This is a SQLite file containing:
...
```bash
clawsmith cost --day      # Today only
clawsmith cost --week     # Current week (default)
clawsmith cost --month    # Current month
...
clawsmith uses token counts from `sessions.json` and a built-in model price table (USD per 1M tokens). The calculation:

```
cost = (inputTokens / 1,000,000 × inputPrice) + (outputTokens / 1,000,000 × outputPrice)
```

Built-in prices include: OpenAI GPT-5.4 / GPT-5.4-mini, Anthropic Claude Sonnet 4.5 / Opus 4, Google Gemini 3.1 Flash, DeepSeek V3, and more. Prices are updated with each clawsmith release.

> **Note**: If your model is not in the price table, clawsmith will display token counts but show cost as `n/a`. You can add custom prices in `~/.clawsmith/config.json` — see [Configuration](#11-configuration).

### Cost alerts

Set a daily budget in your config and clawsmith will alert you when you're approaching it:

```json
// ~/.clawsmith/config.json
{
  "alerts": {
    "dailyBudgetUsd": 5.00
  }
...
In OpenClaw, a **session** corresponds to a single conversation window — one `clawsmith start` to `exit`. Each session has its own `.jsonl` transcript file and a `sessionKey` (e.g., `sess_abc123`). Sessions accumulate token counts independently.
...
```bash
clawsmith status --json | grep sessionKey
# or just:
clawsmith status
...
### Important: clawsmith must be running to capture turn granularity

Turn-level timeline is built from **snapshots** clawsmith takes every time `sessions.json` updates (after each turn). If clawsmith's daemon was not running during part of the session, those turns are merged into a single `[clawsmith was offline]` gap entry:

```
    Turn  1  09:12   +2,100 in /   +380 out   $0.037
    Turn  2  09:18   +3,200 in /   +510 out   $0.058
    ···      [clawsmith offline 09:20 → 11:00]
...
clawsmith detects every compact event from the `.jsonl` transcript files and shows you exactly what was compacted.
...
──────────────────────────────────────────
  ...
  → Save these to memory: clawsmith memory save-compact 3
...
```bash
clawsmith compacts --last 10     # Show last 10
clawsmith compacts --last 3      # Show last 3 (faster)
```

### Show full message content

By default, clawsmith shows a preview. Use `--show-messages` to see full content:

```bash
clawsmith compacts --last 1 --show-messages
```

### Filter by session

```bash
clawsmith compacts --session sess_abc123
```

### Save compacted messages to long-term memory

When clawsmith detects that important content was lost in a compact event, you can save it to `MEMORY.md` with one command:

```bash
clawsmith memory save-compact 3
```
...
```bash
clawsmith memory save-compact 3 --file workspace/memory/2026-03-17.md
```

### How clawsmith detects "important losses"

clawsmith compares the compacted messages against the generated summary. Content is flagged as potentially important if it contains:
...
clawsmith provides a full-featured memory browser and editor on top of OpenClaw's `MEMORY.md` and `memory/YYYY-MM-DD.md` files.
...
```bash
clawsmith memory list --file workspace/memory/2026-03-17.md
```

### Search memory

```bash
clawsmith memory search "database"
```
...
```bash
clawsmith memory add "API pagination: default 20 items per page, max 100"
```
...
```bash
clawsmith memory add "Meeting notes: backend refactor planned for Q2" \
...
```bash
clawsmith memory edit 3 "TypeScript: strict mode + no implicit any + no unused vars"
```

### Delete a memory entry

```bash
clawsmith memory delete 3
```
...
When you add, edit, or delete memory entries, clawsmith writes directly to the Markdown file. OpenClaw monitors these files with its own file watcher and will pick up changes automatically — no restart needed.
...
clawsmith maintains this format to stay compatible with OpenClaw's parser.
...
Common offender: `TOOLS.md`. If you have many tools registered, their descriptions can easily exceed 20,000 chars. clawsmith detects this and tells you exactly how much is being cut.
...
clawsmith continuously analyzes your agent's behavior and flags issues.
...
```bash
clawsmith suggest --severity critical     # Only critical
clawsmith suggest --severity warning      # Warning and above
```
...
```bash
clawsmith suggest --dismiss tools-truncation
```
...
If clawsmith is running on a remote server:
...
Install the clawsmith Skill to let your OpenClaw agent answer questions about its own context, costs, and memory — directly in chat. Works with any messaging app connected to OpenClaw (Telegram, WhatsApp, Slack, WeChat, etc.).

### Install the Skill

```bash
openclaw plugins install @clawsmith/skill
```

This registers clawsmith's tools with your OpenClaw agent. The clawsmith daemon must be running (`clawsmith start`).
...
Agent: 💡 1 critical, 1 warning:
       ...
       Run `clawsmith suggest` for details
```
...
clawsmith works with zero configuration in most cases. To customize behavior, create `~/.clawsmith/config.json`.
...
| `CLAWSMITH_PORT` | Web dashboard port | `4747` |
| `CLAWSMITH_HOST` | Web dashboard host | `127.0.0.1` |
| `CLAWSMITH_DIR` | clawsmith data directory | `~/.clawsmith` |

---

## 13. Troubleshooting

### clawsmith can't find OpenClaw
...
```bash
OPENCLAW_DIR=/custom/path/.openclaw clawsmith status
```

Or add to `~/.clawsmith/config.json`:
...
clawsmith reads from `sessions.json`, which OpenClaw writes after each turn. If OpenClaw hasn't written the file yet, token counts may lag by one turn. Wait a few seconds and run again.
...
Your model isn't in clawsmith's price table. Add a custom price:
...
clawsmith needs to parse the `.jsonl` transcript files. Check that they exist:
...
```bash
clawsmith compacts --session <your-session-key>
```
...
OpenClaw reads workspace files at session start and during context assembly. After editing via clawsmith, the changes will apply in the next context cycle. For immediate effect, start a new session.

If edits are being overwritten, check if another process (or OpenClaw itself) is writing to the same file.

### Permission errors

clawsmith must have read access to `~/.openclaw` and write access to `~/.clawsmith`. Check permissions:
...
```
clawsmith memory list [options]
  --file <path>       Target file (default: MEMORY.md)
  --agent <name>      Target agent

clawsmith memory search <query> [options]
  --agent <name>      Target agent
  --limit <n>         Max results (default: 10)

clawsmith memory add <content> [options]
  --file <path>       Target file (default: MEMORY.md)
  --agent <name>      Target agent

clawsmith memory edit <entry-id> [content] [options]
  --file <path>       Target file
  --agent <name>      Target agent
  (if content omitted, opens $EDITOR)

clawsmith memory delete <entry-id> [options]
  --file <path>       Target file
  --agent <name>      Target agent
  --yes               Skip confirmation

clawsmith memory save-compact <compact-id> [options]
  --file <path>       Target file (default: MEMORY.md)
  --agent <name>      Target agent
```
...
```
clawsmith session [session-key] [options]
...
```bash
clawsmith session                    # Active session summary + turn timeline
clawsmith session --list             # All sessions table
clawsmith session sess_abc123        # Specific session by key
clawsmith session sess_abc123 --json # JSON output for scripting
clawsmith session --no-turns         # Summary only (no turn timeline)
```
...
*For questions, issues, or contributions, visit [github.com/your-username/clawsmith](https://github.com/your-username/clawsmith)*
