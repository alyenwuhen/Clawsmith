...
clawsmith はこの問題を解決します。バックグラウンドで OpenClaw のファイルを監視し、エージェントが今何をしているかをリアルタイムで可視化します。

| あなたの疑問 | clawsmith の答え |
|------------|----------------|
| 「エージェントは今、正常に動いている？」 | `clawsmith status` — 即時スナップショット |
| 「ずっと見ていたい」 | `clawsmith top` — 自動更新のライブダッシュボード |
| 「なぜコンテキストがすぐ圧縮されるの？」 | `clawsmith context` + `clawsmith suggest` |
| 「圧縮後にエージェントが忘れたことは？」 | `clawsmith compacts` |
| 「これ、いくらかかってるの？」 | `clawsmith cost --week`（主要モデルの価格内蔵） |
...
clawsmith start    # バックグラウンドデーモンを起動（OpenClaw を自動検出）
clawsmith status   # 即時スナップショットを確認
```

clawsmith は OpenClaw のインストール先を自動検出します。API キー不要、アカウント登録不要、テレメトリなし。

### OpenClaw スキルとしてインストール（1 コマンド）

OpenClaw を使用している場合、clawsmith をスキルとしてインストールすることで、エージェントが自分自身を監視できるようになります：
...
> https://raw.githubusercontent.com/seekcontext/Clawsmith/main/skills/clawsmith/SKILL.md を読み、その手順に従って clawsmith のセルフモニタリングを設定してください。

新しい OpenClaw セッションを開始すると、エージェントは自動的に `clawsmith` コマンドにアクセスできるようになります。スキルの完全な定義は [`skills/clawsmith/SKILL.md`](./skills/clawsmith/SKILL.md) を参照してください。

---

## コマンド一覧

### `clawsmith status` — 即時スナップショット
...
──────────────────────────────────────────────────
  ...
  Today:     $0.12  → clawsmith cost で詳細を確認
...
```bash
clawsmith top                  # デフォルト 2 秒更新
clawsmith top --interval 5     # 5 秒更新
clawsmith top --agent coder    # 特定のエージェントを指定
```

---

### `clawsmith cost` — API コスト追跡
...
内蔵済み価格：OpenAI（GPT-4o、o1、o3、o4-mini）、Anthropic（Claude 3/3.5/3.7 Sonnet/Opus/Haiku）、Google（Gemini 2.0/2.5 Flash/Pro）、Moonshot（kimi-k2.5）、DeepSeek（v3、r1）など。未収録モデルは `~/.clawsmith/config.json` でカスタム追加できます。

---

### `clawsmith session` — セッション詳細
...
### `clawsmith context` — コンテキストウィンドウ分析
...
不要なルールを無効化：`clawsmith suggest --dismiss <rule-id>`

---

## エージェント連携

clawsmith は**エージェントから呼び出されること**を想定して設計されています。すべてのコマンドが `--json` 出力をサポートし、エラーも常に構造化 JSON で返ります。パースを壊すカラーテキストは出力しません。
...
```bash
clawsmith schema           # 全コマンドを一覧表示
clawsmith schema status    # status --json のフィールド仕様
clawsmith schema cost      # cost --json のフィールド仕様
```
...
```bash
clawsmith session --json   # アクティブなセッションがない場合
...
オプション設定ファイル `~/.clawsmith/config.json` —— 初回の `clawsmith start` 実行時に自動生成されます：
...
ほとんどのユーザーに設定は不要です。clawsmith は既存の OpenClaw 設定からすべてを自動検出します。

---

## 仕組み

clawsmith はバックグラウンドで OpenClaw の既存ファイルを読み取ります —— コード変更・プラグイン・フック一切不要。

- **設定不要** — `~/.openclaw` の OpenClaw を自動検出
- **副作用ゼロ** — OpenClaw のファイルには一切触れず、`~/.clawsmith/` のみに書き込む
- **バックグラウンドデーモン** — `clawsmith start` がファイル変更を監視し、ローカル DB を随時更新
