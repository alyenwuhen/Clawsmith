...
/**
 * Write a commented config template to ~/.clawsmith/config.json if it does not already exist.
...
export function initConfigTemplate(probeDir: string): boolean {
  ...
  const template = {
    _comment: "clawsmith config — https://github.com/openclaw/clawsmith",
    openclaw: {
      dir: path.join(os.homedir(), ".openclaw"),
      agent: "main",
    },
  ...
  if (!fs.existsSync(cfg.openclawDir)) {
    console.error(
      `Error: OpenClaw directory not found at ${cfg.openclawDir}\n` +
        `Set OPENCLAW_DIR environment variable or add "openclaw.dir" to ~/.clawsmith/config.json`
    );
    process.exit(1);
  }
}
