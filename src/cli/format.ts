...
// Priority: TZ env var → ~/.clawsmith/config.json "timezone" → /etc/localtime symlink → Intl default
function resolveTimezone(): string {
  if (process.env.TZ) return process.env.TZ;

  // Try ~/.clawsmith/config.json "timezone" field
  try {
    const configPath = osSync.homedir() + "/.clawsmith/config.json";
