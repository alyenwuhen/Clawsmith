...
export async function runStatus(cfg: ResolvedConfig, opts: StatusOptions): Promise<void> {
  ...
  const daemonLine = daemonRunning
    ...
    : severity.warning("  ○ daemon not running  →  run: clawsmith start");
  ...
  if (todaySummary.totalUsd > 0) {
    console.log(`  Today:     ${fmtUsd(todaySummary.totalUsd)}  ${severity.muted("→ clawsmith cost for full breakdown")}`);
  } else {
  ...
  if (suggestions.length > 0) {
    ...
    if (suggestions.length > 3) {
      console.log(severity.muted(`     … +${suggestions.length - 3} more  →  clawsmith suggest`));
    } else {
      console.log(severity.muted(`     →  clawsmith suggest  for details & actions`));
    }
  }
