...
export async function runCost(cfg: ResolvedConfig, opts: CostOptions): Promise<void> {
  ...
  if (summary.daily.length === 0) {
    console.log(severity.muted("  No cost data yet. Run clawsmith as a daemon to collect data."));
    console.log();
    return;
  }
  ...
  if (summary.unpricedModels && summary.unpricedModels.length > 0) {
    ...
    console.log(severity.muted(`    Add to ~/.clawsmith/config.json → cost.customPrices to enable cost tracking.`));
  }
