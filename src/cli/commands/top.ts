...
function render(cfg: ResolvedConfig, agent: string, intervalSec: number): void {
  ...
  // ── Row 1: title bar ───────────────────────────────────────────────────────
  const ts       = nowStr();
  const titleText = `clawsmith top  refreshing every ${intervalSec}s  (q / Ctrl+C to quit)`;
  const titlePad = Math.max(0, W - titleText.length - ts.length);
  writeLine(1,
    chalk.bold("clawsmith top") +
  ...
  const daemonBadge = daemonRunning
    ...
    : severity.warning("○ daemon stopped — run: clawsmith start");
  ...
  if (suggestions.length > 2) {
    alertLines.push(chalk.dim(`     … +${suggestions.length - 2} more  →  clawsmith suggest`));
  }
