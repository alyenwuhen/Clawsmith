# clawsmith — Technical Design Document
...
clawsmith is a local-first observability and management tool for OpenClaw AI agents. It reads OpenClaw's existing data files (session transcripts, session store, workspace memory files) without modifying any OpenClaw internals, and provides:
...
`sessions.json` stores **cumulative** token counts for the lifetime of a session. To calculate the cost of a **single session** (conversation), clawsmith uses snapshot diffing:
...
**Why snapshot diff works**: When a session starts, clawsmith writes the first snapshot (tokens = 0 or carry-over from prior compaction). Every time `sessions.json` updates, a new snapshot is written. The delta between first and last snapshot gives the cost attributable to that session.

**Limitation**: If clawsmith was not running when the session started, the earliest snapshot may not capture the true session start. In that case, clawsmith shows a note: `"Cost may be understated (clawsmith was not running at session start)"`.

**Turn-level granularity**: Each snapshot in `session_snapshots` is timestamped. clawsmith can show a **turn-by-turn cost timeline** by computing deltas between consecutive snapshots within a session.
...
**Usage**: Query for memory search (FTS5 full-text search). clawsmith opens this in read-only mode (`SQLITE_OPEN_READONLY`) to avoid any write conflicts with OpenClaw.

```sql
-- clawsmith uses OpenClaw's own FTS5 index
...
## 4. clawsmith SQLite Schema

clawsmith maintains its own analysis database at `~/.clawsmith/probe.db`.
...
```
clawsmith <command> [subcommand] [options]
```

### Commands

```
clawsmith start              Start the background daemon + web dashboard
clawsmith stop               Stop the daemon
clawsmith status             Current session status (tokens, model, compactions)
clawsmith cost               Cost summary (default: current week)
clawsmith session            Per-session cost and turn breakdown
clawsmith compacts           Recent compaction events
clawsmith memory             Memory management subcommands
clawsmith context            Context composition estimate
clawsmith suggest            Optimization suggestions
clawsmith config             Show detected OpenClaw configuration
clawsmith version            Show clawsmith version
```
...
clawsmith memory search <query>
  --agent <name>     Target agent
  --limit <n>        Max results (default: 10)

clawsmith memory add <content>
  --file <path>      Target file (default: MEMORY.md)
  --agent <name>     Target agent

clawsmith memory edit <entry-id>
  --file <path>      Target memory file
  --agent <name>     Target agent

clawsmith memory delete <entry-id>
  --file <path>      Target memory file
  --agent <name>     Target agent

clawsmith memory save-compact <compact-id>
  --file <path>      Target memory file (default: MEMORY.md)
  --agent <name>     Target agent
...
```
skills/clawsmith/
├── SKILL.md        Teaches the agent how to use clawsmith tools
├── tools.json      Tool definitions
└── probe-tool.sh   Shell script calling clawsmith CLI
```
...
clawsmith is an observability tool installed on this machine.
Use these tools to answer questions about token costs, memory, and context.

Available commands (run via exec tool):
- `clawsmith status --json`       Current session status
- `clawsmith cost --week --json`  This week's API cost
- `clawsmith compacts --json`     Recent compaction events
- `clawsmith suggest --json`      Optimization suggestions
- `clawsmith memory search <q>`   Search agent memory
- `clawsmith memory add <text>`   Add to long-term memory
```

**Supported IM commands** (user sends to OpenClaw, which calls clawsmith):
...
│   │   ├── db.ts             clawsmith SQLite (probe.db)
...
```bash
curl -fsSL https://clawsmith.dev/install.sh | bash
```
...
- [ ] `clawsmith status` — session status with token bar
- [ ] `clawsmith cost --week` — weekly cost breakdown
- [ ] `clawsmith compacts --last 5` — compact event list
- [ ] `clawsmith context` — workspace file size analysis
- [ ] `clawsmith memory list/search/add/edit/delete`
- [ ] `clawsmith suggest` — basic rule engine
